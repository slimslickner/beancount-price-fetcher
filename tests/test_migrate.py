...
def test_migrate_two_dated_files_layout_exact_example(tmp_path: Path) -> None:
    ...
    aapl = prices / "AAPL.beancount"
    spy = prices / "SPY.beancount"
    assert aapl.exists()
    assert spy.exists()
...
def test_migrate_two_dated_files_loads_with_zero_errors(tmp_path: Path) -> None:
    ...
    for commodity in ("AAPL", "SPY"):
        entries, errors, _ = parser.parse_file(str(prices / f"{commodity}.beancount"))
...
def test_migrate_two_dated_files_idempotent(tmp_path: Path) -> None:
    ...
    content = (prices / "AAPL.beancount").read_text()
...
def test_migrate_two_dated_files_dedup_same_date_two_commodities(
    ...
    aapl_lines = [
        ln for ln in (prices / "AAPL.beancount").read_text().splitlines() if ln.startswith("2026")
    ]
...
def test_migrate_two_dated_files_three_commodities(tmp_path: Path) -> None:
    ...
    assert set(p.name for p in prices.iterdir() if p.suffix == ".beancount") >= {
        "AAPL.beancount",
        "SPY.beancount",
        "MSFT.beancount",
    }
